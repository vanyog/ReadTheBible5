...
public:
   ...
   shortTitles, // Кратки заглавия на книгите, както се показват в падащия списък за избиране.
   ...
   */
   bool synchronize,  // Определя дали прозорецът да се синхронизира с активния стих
       ...
       notOnScroll = false; // Предотвратява изпалняването на onScroll

   BibleWindow( const QString &bv, QWidget *parent = 0); // Конструктор на класа. bv е идентификатор на версия на Библията

   QString bibleVersion(); /* Идентификатор на версията на Библията, който съвпада с името на директорията ѝ.*/
   ...
   QString reference(int i, bool abr = false); // Връща препратка към стих с локален индекс i. Когато abr=true името на книгата е съкратено
   ...
   void thereIsNewerToDownload(); // Съобщение, че има по-нов файл за изтегляне
...
private:
   ...
   bool force_Set_ReadPos; // Флаг, който определя текущия стих да се запази като място за четене
   bool isLink; // Мешката кликва върху линк
   ...
   bool activateOnClicked(); // Активира се стихът, върху който се кликва.
   ...
   bool isActiveMdiWindow(); // Определя дали е активен прозорец
};

//----------функции-------------

QString versionCaption(const QString &bv); // По идентификатора на версията връща заглавието ѝ.

QByteArray versionCodec(const QString &bv); // По идентификатора на версията връща кодовата ѝ таблица.

QString versionLanguage(const QString &bv); // По идентификатора на версията връща езика ѝ.
...
/* Запазва информацията за версия на Библията с идентификатор bv.
   ...
   Тази информация се намира в редовете на файл data/bibles/list-u.txt и се съхранява в хешове.
